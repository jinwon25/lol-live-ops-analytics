...
- 분석이 너프/버프 후보로 라벨링한 챔프가 라이엇 V15.3 공식 패치 노트와 **7건 일치**했습니다 (정밀 매칭 6건 + 예측 적중 1건).
...
| 게임 시스템 | 솔로듀오 / `queue_id=420` | 1인 또는 2인 단위로 참여하는 LoL 표준 랭크 큐 (소환사의 협곡 5v5) — Riot API에서 `queue_id=420`으로 식별 |
...
| 게임 용어 | 너프 / 버프 | 챔프 성능을 약화 / 강화하는 패치 조정. 본문에서는 한국어를 사용하되, 코드 식별자(`flag='NERF'` · `flag='BUFF_HIDDEN_STRONG'`)는 영문 그대로 유지 |
| 게임 용어 | 픽률 상승 / 픽률 하락 | 패치 사이 챔프 선택 빈도가 늘었거나 줄어든 상태 (이전 표기 "떠오름 / 저묾"의 풀어쓴 표현) |
| 게임 용어 | 사장된 강캐 | 픽률은 낮아 잘 선택되지 않지만 승률은 높은 챔프. 표본이 작아 점추정 대신 Wilson 신뢰구간 하한으로 판별 (`BUFF_HIDDEN_STRONG`) |
| 게임 용어 | 스노우볼 | 게임 초반 격차가 후반까지 가속되며 한쪽으로 기우는 현상 |
| 스탯 약어 | AD / AP | 공격력(Attack Damage) / 주문력(Ability Power) |
| 스탯 약어 | AS / CD | 공격속도(Attack Speed) / 쿨다운(Cooldown) |
| 스탯 약어 | base AD/armor | 기본(base) 공격력 / 방어력 — 1레벨 시점 수치 |
| 스탯 약어 | AP 계수 (AP비) | 스킬 피해량이 주문력 1당 증가하는 비율 |
| 스킬 슬롯 | 패시브 · Q · W · E · R | 챔프의 5개 스킬 키. Q~R 순으로 키보드 단축키 |
...
- **자동 너프 / 버프 플래깅** — 인기 NTILE 분위 × 표본 컷 × 강함 컷의 조합으로 라벨링합니다. 버프 후보(사장된 강캐)는 정의상 표본이 작으므로 점추정 대신 **Wilson 95% 신뢰구간 하한 ≥ 50%** 로 강함을 정의했고, SQL로 직접 구현했습니다.
...
분석이 코호트(PLAT+EM+DIA)에서 너프 / 버프 후보로 라벨링한 챔프를 라이엇 공식 패치 노트와 대조했습니다. 패치 노트의 영문 스킬·룬 명은 챔프별 스킬 슬롯(Q · W · E · R · 패시브)으로 풀어 표기했습니다.
...
| 미스 포츈 | 픽률 하락 8위, 픽률 −0.43pp | 너프 — 기본 방어력 28→25 | 정확 |
| 스카너 | 픽률 하락 11위 + 승률 −12.8pp | 너프 — 방어력 성장 감소, Q 스킬 기절 지속시간 1.5초 → 1.1초 | 정확 |
| 케인 | 픽률 상승 7위, 픽률 +0.40pp | 버프 — Q 스킬(거두는 일격) 피해량 증가 | 정확 |
| 퀸 | naïve 11위 (교란 효과 사례) | 버프 — W 스킬(고양된 감각) 공격속도 60% → 80% | 정확 |
| **스웨인** | **15.1 사장된 강캐 후보** (`BUFF_HIDDEN_STRONG`, MID, n=21, win 81%, Wilson 60%) | **15.3 버프** — 패시브(탐욕의 무리) 영혼당 추가 체력 12 → 15, E 스킬(굳건한 의지) 주문력 계수 70% 증가 | **예측 적중** |

스웨인 사례는 사후 매칭이 아닙니다. 15.1 데이터에서 버프 후보가 처음에 0건이 나왔을 때 "사장된 챔프는 표본이 적다"는 정의의 본질과 컷이 자기모순이라는 점을 진단하고, Wilson 신뢰구간 하한 기반으로 컷을 재설계해 발견한 챔프를 라이엇이 다음 패치에서 실제로 버프했습니다. (상세 흐름: `docs/02_phase2_notes.md §9.2`)

매칭되지 않은 신호도 그대로 기록했습니다. 신 짜오 승률 +25.5pp는 15.3 패치 노트에 변경이 없어 표본 노이즈로 확정했습니다.
...
| 퀸 | 11위 | top15 밖 | naïve 우대, 그러나 라이엇 실제 버프 |
...
| 액션 | 근거 | 산출물 |
|---|---|---|
| 다음 패치 너프 후보 자동 플래깅 (총 85건) | 인기 quartile=4 + n≥30 + WR≥52% | `outputs/champion_meta.csv` flag 컬럼 (`flag='NERF'`) |
| 다음 패치 버프 후보 자동 플래깅 (총 10건) | quartile≠4 + n≥15 + Wilson 하한≥50% | 동상 (`flag='BUFF_HIDDEN_STRONG'`) |
...
│   ├── 12_flagging.sql      # 너프 / 사장된 강캐(BUFF_HIDDEN_STRONG) 라벨링
...
- **자동 플래깅 규칙의 사각지대** — 픽률↓ + 승률↑ 패턴("사장 중인 강캐")이 현 버프 규칙(quartile≠4)에 잡히지 않는 케이스(예: 잔나)가 존재합니다. 다음 버전 규칙은 *픽률 추세 + 승률 추세 결합* 라벨(`FADING_STRONG` / `CLIMBING_WEAK`)로 확장할 예정입니다. (`docs/02_phase2_notes.md §6.5–6.6`)
...
| [docs/02_phase2_notes.md](docs/02_phase2_notes.md) | 메타 SQL · 스웨인 인과 사슬 · 잔나 사각지대 · 패치노트 매칭 |
